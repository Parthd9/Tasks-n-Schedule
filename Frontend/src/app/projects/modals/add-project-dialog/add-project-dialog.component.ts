...
export class AddProjectDialogComponent implements OnInit {
  ...
  constructor(private dialogRef: MatDialogRef<AddProjectDialogComponent>,  @Inject(MAT_DIALOG_DATA) data, private projectService: ProjectsService) {
    ...
    this.addProjectForm = new FormGroup({
      'name': new FormControl('', [Validators.required, Validators.minLength(3), Validators.maxLength(28)]),
      'description': new FormControl('', [Validators.required, Validators.minLength(20), Validators.maxLength(200)]),
      'team': new FormControl('', [Validators.required, Validators.minLength(3)]),
    ...
    obs.subscribe(result => {
      if(result['status'] == 201 || result['status'] == '202') {
        ...
        this.dialogRef.close({event:'success',value:result['body']['data']});
          ...
          projectDoc = {name: this.addProjectForm.get('name').value, description: this.addProjectForm.get('description').value, 
          team: this.addProjectForm.get('team').value, technologies:this.addProjectForm.get('technologies').value,
