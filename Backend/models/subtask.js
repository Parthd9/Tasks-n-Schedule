...
class SubTask {
  ...
  static editSubTask(orgId, subtaskId, description) {
    ...
    return db
      .collection("subtasks")
      .updateOne(
        { _id: new ObjectId(subtaskId), orgId: orgId },
  ...
  static removeSubtask(orgId, subtaskId) {
    ...
    return db
      .collection("subtasks")
      .deleteOne({ orgId: orgId, _id: new ObjectId(subtaskId) });
  }
  ...
  static getDocumentCount(orgId, taskId) {
    ...
    return db
      .collection("subtasks")
      .find({ orgId: orgId, taskId: taskId })
      .count();
  }
  static subtaskComplete(orgId, subtaskId, completionTime) {
    ...
    return db
      .collection("subtasks")
      .updateOne(
        { orgId: orgId, _id: new ObjectId(subtaskId) },
        { $set: { completionTime: completionTime, isCompleted: true } }
      );
  }
  static getCompletedDocumentCount(orgId, taskId) {
    ...
    return db
      .collection("subtasks")
      .find({ orgId: orgId, taskId: taskId, isCompleted: true })
      .count();
  }
  static saveSpentTime(orgId, taskId, sId, time, taskDetails, type) {
    const db = database.getDb();
    if (type === "Add") {
      return db.collection("backlog-subtask").insertOne({
        orgId: orgId,
        taskId: taskId,
        sprintId: sId,
        spentTime: time,
      ...
      return db
        .collection("backlog-subtask")
        .updateOne(
          { orgId: orgId, taskId: taskId, sprintId: sId },
          { $set: { spentTime: time, taskDetails: taskDetails } }
        );
    }
  }
  static updateStatusInTaskDetail(orgId, taskId, sprintId, status) {
    ...
    return db
      .collection("backlog-subtask")
      .updateOne(
        { orgId: orgId, taskId: taskId, sprintId: sprintId },
        { $set: { "taskDetails.status": status } }
      );
  }
  ...
  static getSpentTime(orgId, taskId) {
    ...
    return db
      .collection("backlog-subtask")
      .findOne({ orgId: orgId, taskId: taskId });
  }
  static deleteSpentTime(orgId, taskId) {
    const db = database.getDb();
    return db
      .collection("backlog-subtask")
      .deleteOne({ orgId: orgId, taskId: taskId });
  }
