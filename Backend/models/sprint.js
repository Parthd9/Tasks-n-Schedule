...
class Sprint {
  ...
  static getSprints(orgId, email, projectId, versionId) {
    ...
    return db
      .collection("sprints")
      .find({ orgId: orgId, projectId: projectId, versionId: versionId })
      .project({ creator: 1, description: 1, name: 1, completionDate: 1 })
      .toArray();
  }
  ...
  static getMailList(orgId, pId) {
    ...
    return db
      .collection("projects")
      .find({ orgId: orgId, _id: new ObjectId(pId) })
      .project({ team: 1, creator: 1 })
      .toArray();
  }
}
